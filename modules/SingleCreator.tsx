...
const SingleCreator: FC<{ singleCreateData: ArtistType; id: string }> = ({
  singleCreateData,
  id,
}) => {
  const { setSingleAritstData } = useContext(Context);
  const { data: SingleArtist }: { data: ArtistType } = getQueryData(
    `/user/${id}`,
    singleCreateData,
    "single_creator",
    id
      ...
      <div className="containers">
        ...
        <div className="flex">
          <Link
            href={`/creator/${id}`}
            className={`${
              !pathname.includes("owned") && !pathname.includes("collections")
                ? "border-[#858584]"
                : "border-transparent"
            } border-b-[2px] w-[33%] flex items-center cursor-pointer gap-[16px] justify-center py-[15px] text-center`}
          >
            <span className="text-[22px] text-white leading-[140%]">
              Created
            </span>
            <button className="bg-[#858584] text-white py-[5px] px-[10px] rounded-[20px]">
              {SingleArtist.createdNFTs.length}
            </button>
          </Link>
          <Link
            href={`/creator/${id}/owned`}
            className={`${
          ...
          >
            ...
            <button className="bg-[#858584] text-white py-[5px] px-[10px] rounded-[20px]">
              {SingleArtist.ownedNFTs.length}
            </button>
          </Link>
          <Link
            href={`/creator/${id}/collections`}
            className={`${
          ...
          >
            ...
            <button className="bg-[#858584] text-white py-[5px] px-[10px] rounded-[20px]">
              {SingleArtist.collections.length}
            </button>
          </Link>
        </div>
      </div>
