...
const MushroomsContent = ({
  mushroomData,
}: {
  mushroomData: { data: AuctionsType2[]; meta: MetaType };
}) => {
  ...
  return (
    <section
      ...
      className="h-[660px] flex justify-end pb-[60px] overflow-hidden"
    >
      <div className="containers flex items-end w-full justify-between">
        <div>
          <Link
            href={`/creator/${mushroomData.data[0].nft.creator.id}`}
            className="flex items-center space-x-3 bg-[#3B3B3B] rounded-[20px] w-[151px] py-[15px] px-[20px]"
          >
            <Image
            ...
            <span className="text-white font-medium">
              {mushroomData?.data[0]?.nft?.creator?.username}
            </span>
          </Link>
          <h1 className="text-5xl my-[30px] font-bold text-white mb-8 bg-gradient-to-r from-white to-purple-200 bg-clip-text">
            {mushroomData.data[0].description}
          </h1>
