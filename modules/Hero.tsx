...
const Hero: FC<HeroType> = ({ actions, artists, actionResults }) => {
  ...
  const { data: actionsList, isLoading: actionLoading } = getQueryData(
    "/auctions",
    actions,
    "auctions"
  );
  const { data: artistsList, isLoading: artistLoading } = getQueryData(
    "/user?role=ARTIST",
    artists,
    "artist"
  );
  const { data: actionResultsList, isLoading: actionResultsLoading } =
    getQueryData("/auction-result", actionResults, "auction-results");
  return (
    <section className="py-[80px]">
      <div className="containers  flex flex-col md:flex-row gap-[10px] justify-between">
        <div className="md:w-[50%]">
          ...
          <Text classList="!text-[16px] lg:!text-[22px] !mb-[20px] lg:!mb-[30px]">
            {t("text")}
          </Text>
          <Button
            extraClass="!px-[50px]"
            title={t("buttonContent")}
            icon={<RocketIcon />}
            iconPostion="left"
            type="button"
            variant="filled"
        ...
        <div className="mx-auto md:mx-0 mt-[40px] md:mt-0 md:w-[330px] lg:w-[510px] self-start bg-[#3B3B3B] rounded-[20px] overflow-hidden">
          <Image
            className="w-[510px] h-[221px] object-cover lg:h-[401px]"
            src={"/hero-img.png"}
            alt="Hero img"
            width={510}
            height={401}
          ...
          <div className="py-[22px] px-[20px]">
            <Heading classList="!text-[22px]" tag="h3">
              Space Walking
            </Heading>
