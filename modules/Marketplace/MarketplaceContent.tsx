...
}> = ({ collectionsList, nftList }) => {
  const [loading, setLoading] = useState<boolean>(false);

  const [value, setValue] = useState<string>("");
  const [searchDataNft, setSearchDataNft] = useState<NftType2[] | null>(null);
  const [searchDataCollection, setSearchDataCollections] = useState<
    CollectionType[] | null
  >(null);
  ...
  const routeList = [
    {
      id: 1,
      title: "NFTs",
      count: searchDataNft ? searchDataNft.length : nftList.length,
      path: "nfts",
    },
    {
      id: 2,
      title: "Collections",
      count: searchDataCollection
        ? searchDataCollection.length
  ...
  return (
    <>
      <div className="containers !pt-[80px]">
        ...
        <Text classList="!text-[22px] !mb-[30px]">
        ...
        <label className="relative mb-[80px] block">
          <Input
            ...
            classList="!bg-transparent border-[1px] w-full border-[#858584] pr-[50px] !text-[#858584]"
            type="text"
        ...
        <div className="containers">
          {active == "nfts" ? (
            loading ? (
              ...
              <Nfts nftList={searchDataNft ? searchDataNft : nftList} />
            )
          ) : (
            ""
          )}
          {active == "collections" ? (
            loading ? (
              ...
              <MarketCollection
                collectionsList={
                  searchDataCollection ? searchDataCollection : collectionsList
                }
              />
            )
          ) : (
